...
inline tensor broadcast_width_height(std::size_t top_pad, std::size_t bottom_pad,
    std::size_t left_pad, std::size_t right_pad,
    const tensor& in){

    assertion (((top_pad != 0 || bottom_pad != 0 || left_pad != 0 || right_pad != 0) && (in.shape().height_ == 1 && in.shape().width_ == 1)) || (top_pad == 0 && bottom_pad == 0 && left_pad == 0 && right_pad == 0), "Should broadcast with a tensor with depth and width 1");
    ...
    if (top_pad == 0 && bottom_pad == 0 && left_pad == 0 && right_pad == 0)
    {
      ...
      in.shape().depth_), in.shape().rank()), 0.0f);

      auto input_ptr = &in.get_ref_ignore_rank(tensor_pos(0, 0, 0, 0, 0));
    ...
    for (std::size_t y = 0; y < result.shape().height_; ++y)
    {
        ...
        {
            auto result_ptr = &result.get_ref_ignore_rank(tensor_pos(0, 0, y, x, 0));
...
inline tensor multiply_tensors(const tensors& ts_all)
{
    ...
    // ATEME_CNN_QTMT Try to modify the function for the multiply function
    ...
    if(!fplus::all_the_same_on(fplus_c_mem_fn_t(tensor, shape, tensor_shape), ts)){
      ...
      std::size_t max_height = std::max(ts.front().height(), ts.back().height());
      std::size_t max_width = std::max(ts.front().width(), ts.back().width());

      ts.front() = broadcast_width_height(max_height - ts.front().height(), 0, max_width - ts.front().width(), 0, ts.front());

      ts.back() = broadcast_width_height(max_height - ts.back().height(), 0, max_width - ts.back().width(), 0, ts.back());
